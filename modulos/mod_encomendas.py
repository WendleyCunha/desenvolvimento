...
principal. Reúne TUDO que gira em torno do ciclo de vida de um pedido:
  ...
  • Edição completa de um pedido (`_conteudo_pedido`) — reaproveitada pelos
    popups da Agenda, por Contratos e por Gerenciar Pedidos
...
NADA de lógica de negócio mudou nesta extração — é uma cópia fiel do que já
existia em main.py, só reorganizada num módulo próprio (mesmo espírito da
extração de `modulos/mod_financeiro.py`).

DUAS ÚNICAS ADAPTAÇÕES TÉCNICAS feitas para este módulo funcionar sozinho,
sem depender de variáveis globais do main.py:

  1) `dialog_nova_encomenda` e `secao_nova_encomenda_inline` usavam o
     `df_enc_all` (calculado uma vez no main.py) só para desenhar o
     mini-calendário de ocupação da Confecção. Aqui, em vez do global,
     buscamos os dados na hora com `encomendas_listar(cancelado=False)`
     (que já é cacheado por `database.py` — nenhum custo extra de leitura).

  2) As constantes `ETAPAS`, `DIC_MEDIDAS`, `SENHA_DELETE` e `LOGO_PATH`
     passaram a viver aqui (fonte única). O `main.py` importa
     `DIC_MEDIDAS` (usada em Medidas), `SENHA_DELETE` (usada em
     Configurações → Exclusão Permanente) e `LOGO_PATH` (usada no cabeçalho)
     deste módulo, em vez de as redefinir.
