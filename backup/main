...
Histórico de versões (changelog):
  ...
  [v15] MODULARIZAÇÃO — BLOCO ENCOMENDAS: todo o bloco de criação/edição de
        pedido foi extraído para `modulos/mod_encomendas.py` (mesmo espírito
        da extração do BLOCO FINANCEIRO em v11.2): geração de PDF de
        contrato, mini-calendário de ocupação da Confecção, os helpers de
        senha ADM, `dialog_nova_encomenda`, `secao_nova_encomenda_inline`,
        `_conteudo_pedido` e seus popups, `_card_pedido`, e as telas
        `renderizar_contratos`, `renderizar_nova_encomenda` e
        `renderizar_gerenciar_pedidos`. Nenhuma regra de negócio mudou —
