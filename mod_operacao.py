...
def normalizar_picos(df):
    ...
    # Limpeza profunda de nomes de colunas
...
def renderizar_picos_operacional(db_picos, db_data, mes_ref):
    ...
    # BOTÃO DE SEGURANÇA (Caso os dados estejam corrompidos no banco)
    ...
    with st.expander("🛠️ RECUPERAÇÃO DE DADOS"):
        ...
        if st.button("🚨 LIMPAR TODOS OS PICOS DESTE MÊS", use_container_width=True):
            ...
            st.success("Dados limpos! Recarregando...")
    ...
    # Verificação de colunas pós-normalização

    colunas_fatais = [c for c in ['DATA', 'TICKETS', 'HORA', 'DIA_SEMANA'] if c not in df.columns]

    if colunas_fatais:

        st.error(f"❌ Erro de Colunas: A planilha não possui {colunas_fatais}")
    ...
    with tab_dim:
        ...
        if dias_selecionados:
            ...
            df_tab = df_dim.rename(columns={'HORA': 'Hora', 'TICKETS': 'Média Tickets', 'AGENTES_NECESSARIOS': 'Staff Ideal', 'CARGA_POR_AGENTE': 'Tickets/Agente Atual'})
